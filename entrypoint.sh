...
then
    ...
    # dopo il seed importo lingue extra: ciclo sui file desiderati
    for f in data/Database_*.xlsx; do
        # deduco il nome lingua dal file o passo --language-name se serve
        python manage.py import_language_from_excel --file "$f" || echo "Errore su $f"
    done
fi
